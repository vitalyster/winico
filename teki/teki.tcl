...
#-----------------------------------------------------------------------
# Function:	TekiSplashScreen
#			Creates a Splash Window which appears initially
#
#-----------------------------------------------------------------------

proc TekiSplashScreen {} {
    global .splash .menu

    Debug_Enter TekiSplashScreen
	toplevel .about-Teki
	frame .about-Teki.splash -width 100m -height 50m -relief groove -bd 4
	pack .about-Teki.splash -fill both -padx 1m -pady 2m
	message .about-Teki.splash.msg1 -font *-Times-BOLD-I-Normal--*-220-*-*-*-*-*-* -text "TEKI" -width 100m
	message .about-Teki.splash.msg2 -font *-Times-BOLD-R-Normal--*-200-*-*-*-*-*-* -text "(c) Cornell University 1997" -width 100m
	message .about-Teki.splash.msg3 -font *-Times-Light-R-Normal--*-200-*-*-*-*-*-* -text "Brian Smith" -width 50m
	message .about-Teki.splash.msg4 -font *-Times-Light-I-Normal--*-200-*-*-*-*-*-* -text "bsmith@cs.cornell.edu" -width 50m
	message .about-Teki.splash.msg5 -font *-Times-Light-R-Normal--*-200-*-*-*-*-*-* -text "Sumit Kapoor" -width 50m
	message .about-Teki.splash.msg6 -font *-Times-Light-I-Normal--*-200-*-*-*-*-*-* -text "sumit@cs.cornell.edu" -width 50m
	pack .about-Teki.splash.msg1 .about-Teki.splash.msg2 -side top -fill both
	pack .about-Teki.splash.msg3 .about-Teki.splash.msg4 -side top -fill both
	pack .about-Teki.splash.msg5 .about-Teki.splash.msg6 -side top -fill both
	grab set .about-Teki
	after 5000 {destroy .about-Teki}
    Debug_Leave
}
...
proc TekiReadVersion1.0 {tek_fname} {
global pkgid newPackage tekiInfo

Debug_Enter TekiReadVersion1.0 
...
# The third flag has been kept to ease in future additions

set patterns {{" *System *Names *= *" "tekiInfo(systemNames)" 0}
			  {" *Package *Id. *= *" "newPackage(available)" 0}
			  {" *Package *Name *= *" ",name)" 1}
			  {" *Version *= *" ",version)" 1}
			  ...
			  {" *Default *Packages *= *" "newPackage(defaultPackages)" 0}
			  {" *Default *Architecture *= *" "newPackage(defaultArch)" 0}
			  {" *Default *Install *Documentation *= *" "newPackage(defaultInstallDoc)" 0}
			  {" *Default *Install *Examples *= *" "newPackage(defaultInstallExamples)" 0}
			  {" *Default *Install *Data *= *" "newPackage(defaultInstallData)" 0}}
...
# Remove all square brackets so that a malicious user may not enter an executable command
		...
		error "Unable to read version 1.0 TEKI file $filename."
		return
    }        
	...
	foreach i $patterns {
		if {[regexp -indices [lindex $i 0] $fl_buf x] != 0} {
			...
			if {[lindex $i 2] == 0} {
				...
				set varname "newPackage($pkgid[lindex $i 1]"
			}

			if {$j == 17 || $j == 18} {
				set $varname [glob $tstr]
			} elseif {$j == 1} {
				set pkgid $tstr
				lappend $varname $tstr
			} else {
				set $varname $tstr
			}
